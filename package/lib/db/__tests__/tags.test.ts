...
describe('TagService', () => {
  ...
  describe('getTagsByPrefix', () => {
    ...
    it('should return tags ordered alphabetically', () => {
      ...
      for (let i = 0; i < tags.length - 1; i++) {
        expect(tags[i].name.localeCompare(tags[i + 1].name)).toBeLessThanOrEqual(0);
      }
  ...
  describe('getTagsBySnippetId', () => {
    ...
    it('should return tags associated with snippet', () => {
      ...
      const db = getDatabase();
      db.prepare('INSERT INTO snippet_tags (snippet_id, tag_id) VALUES (?, ?)').run(snippetId, tag1.id);
      db.prepare('INSERT INTO snippet_tags (snippet_id, tag_id) VALUES (?, ?)').run(snippetId, tag2.id);
    ...
    it('should return tags ordered alphabetically', () => {
      ...
      db.prepare('INSERT INTO snippet_tags (snippet_id, tag_id) VALUES (?, ?)').run(snippetId, tag1.id);
      db.prepare('INSERT INTO snippet_tags (snippet_id, tag_id) VALUES (?, ?)').run(snippetId, tag2.id);
      db.prepare('INSERT INTO snippet_tags (snippet_id, tag_id) VALUES (?, ?)').run(snippetId, tag3.id);
    ...
    it('should handle snippet with maximum tags (5)', () => {
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        const db = getDatabase();
        db.prepare('INSERT INTO snippet_tags (snippet_id, tag_id) VALUES (?, ?)').run(snippetId, tag.id);
      }
  ...
  describe('associateTagsWithSnippet', () => {
    ...
    it('should associate tags with snippet', () => {
      TagService.associateTagsWithSnippet(snippetId, ['javascript', 'react', 'typescript']);
    ...
    it('should reuse existing tags', () => {
      ...
      expect(tags).toHaveLength(2);
      expect(tags.find(t => t.name === 'javascript')?.id).toBe(existingTag.id);
    });
    ...
    it('should handle duplicate tag names in array', () => {
      TagService.associateTagsWithSnippet(snippetId, ['javascript', 'JavaScript', 'JAVASCRIPT']);
  ...
  describe('removeTagsFromSnippet', () => {
    ...
    it('should not affect other snippets', () => {
      ...
      const result = db.prepare(
        'INSERT INTO snippets (title, content, language, tags) VALUES (?, ?, ?, ?)'
      ).run('Another Snippet', 'console.log("test2")', 'javascript', '');
